...
4. **Layer 4 风险调整排名**：同时输出两个榜单——
   ...
   - `robust_ranking.csv`：默认 `robust_return = E[IRR] − downside_aversion×ES −
     ambiguity_aversion×模型不确定性 − permanent_loss_penalty×永久亏损权重`；
     可在配置中切换 `formula: median_cvar`（`中位 IRR − λ×CVaR` 单一惩罚，
     其余风险指标仅展示，避免三项 Bear 相关惩罚重复扣减）。
...
- 护城河分数当前由 ROIC 持续性/利润率稳定性/超额回报合成，识别不了监管变化、技术替代等
  定性破坏；**路线图**：拆为独立证据项（市占率、留存率、定价权、竞争进入、监管风险、
  资本配置）并附失效条件与证据更新时间。
